...
rElt evaluate_V_iR_stream(int mi, int ni, rElt* level_i, rElt* rm, long lev = LV){
	rElt ans = ZERO;
	for (uint128 k = 0; k < ni; ++k)
		ans = addR(ans, myModMultR(level_i[k], chiR(mi, k, rm, lev), lev), lev);
	return ans;
}
...
void evaluate_circuit(circ* c, int d_matmul, uint128*** coeff, long lev){
	clock_t t = clock();
	uint128 val1, val2;
	int di;
	int in1, in2;
...
//// construct circuit
	...
	c->round_level_st = total_depth-1-(1+d);
	cout << "round level: " << c->round_level_st << endl;
	construct_matmul_circ(c, total_depth, d);
	construct_rnd_circ(c, total_depth - (d+1), 2*d, false);
	cout << "\n circuit construct time is: " << (double)((double) clock()-t)/CLOCKS_PER_SEC << endl;


//// generate input (at layer : total_depth)
	t = clock();
	generate_input(c, 2*myPow(2, d)*myPow(2, d), total_depth, 1);
	cout << "time to randomly generate data is: " << (double) ((double)clock()-(double)t)/(double) CLOCKS_PER_SEC << endl;
...
/// Evaluate the circuit in Z_{p^e}
	evaluate_circuit(c, d+1, partLDpoly, E);
...
//	rElt ri = evaluate_V_iR_stream(2*d, n*n, gateVals[0], z, LV);
...
//// verification of Addition layer
	int two_to_j = 1;
	for(int j = 0; j < d; ++j){
		int i = j + total_depth-(d+1);
		ri=sum_check_bin_treeR(gateVals[i+1], ri, 2*d+j, n*n*two_to_j, &com_ct, &rd_ct, r, poly, betavals, z, E);
		temp=z;
		z=r;
		r=temp;
		two_to_j <<= 1;
...
//// verification of Multiplication layer
	sum_check_matmultR(c, gateVals[total_depth], &(gateVals[total_depth][n*n]), ri, 3*d, n*n*n, &com_ct, &rd_ct, r, poly, betavals, z, &ri0, &ri1, in1matmult, in2matmult, E);
	...
	t=clock();
	ri = evaluate_V_iR(2*d, n*n, Vcopy, z, E);
//	ri = evaluate_V_iR_stream(2*d, n*n, Vcopy, z, E);
	if(!eqTest(ri, ri0)){
		cout << "in very last check, ri != ri0. ri was : ";print_uintR(ri);
		cout << " and ri0 was ";print_uintR(ri0);
		cout << endl;
		//exit(1);
	}
	...
	z=r;
	ri = evaluate_V_iR(2*d, n*n, &(Vcopy[n*n]), z, E);
//	ri = evaluate_V_iR_stream(2*d, n*n, &(Vcopy[n*n]), z, E);
	if(!eqTest(ri, ri1)){
