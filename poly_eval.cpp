...
//static rElt** invCSTR = new rElt*[PRIME*E];	// Keeps Motgomery form of 1,2,...,PRIME-1, t, t+1, ..(total# E*PRIME)  mod PRIME^i 	for each i in {0, 1, ..., E}
...
//// Verifier's check
	st = clock();
	if((!eqTest(addR(poly[0][0],poly[0][1]), ri))){
		cout << "first check in polygen failed when ni was " << (1<<(d+d0)) << endl;
		cout << "ri was ";print_uintR(ri); cout << endl;
		cout << "poly[0][0] was ";print_uintR(poly[0][0]);
		cout << " and poly[0][1] was ";print_uintR(poly[0][1]);
		cout << " and their sum was ";print_uintR(addR(poly[0][0], poly[0][1])); cout << endl;
...
//// Check "reducing to one point"
	...
	rElt correct_out = myModMultR(myModMultR(betavals[0], poly[d0][1]), subR(addC(myModMultR(p0, poly[d0][0]), CST[1][LV]), p0));
	 ...
	 rElt rvalue = extrapR(d, poly[d0], rtmp);		// final result after sum-check
	 *vtime_more += (clock() - st);
	 return rvalue;
}
...
// returns V(z_(i+1)). r: stores z_{i+1}, z: stores z_i.
rElt sum_check_rouding_alphaR(rElt* V, rElt ri, int d0, int d, int dp, int* com_ct, int* rd_ct, clock_t* vtime, clock_t* v_time_more, uint128* LDpoly, rElt* r, rElt** poly, rElt* betavals, rElt* z, long lev = LV){
...
// returns V(z_(i+1)). r: stores z_{i+1}, z: stores z_i.
rElt sum_check_rouding_unifyR(rElt* V, rElt ri, int d0, int d, clock_t* v_time_more, int* com_ct, int* rd_ct, rElt* r, rElt** poly, rElt* betavals, rElt* z, long lev = LV){
	for(int i = 0; i < d+d0+1; ++i){
		r[i] = conv_to_monR(rand_128R(), lev);
		poly[i][0] = poly[i][1] = poly[i][2] = poly[i][3] = ZERO;
	}
...
//// Verifier's check
	st = clock();
	if((!eqTest(addR(poly[0][0],poly[0][1]), ri))){
		cout << "first check in unify failed when ni was " << (1<<(d+d0)) << endl;
		cout << "ri was ";print_uintR(ri); cout << endl;
		cout << "poly[0][0] was ";print_uintR(poly[0][0]);
		cout << " and poly[0][1] was ";print_uintR(poly[0][1]);
		cout << " and their sum was ";print_uintR(addR(poly[0][0], poly[0][1])); cout << endl;
...
//// we omitted the check: poly[d0+1][1] = extrapR(2, poly[d0], r[d])
	...
	rElt correct_out = myModMultR(betavals[0], addR(myModMultR(poly[d0][1], poly[d0+1][0]), poly[d0][0]));
	 ...
	 rElt rvalue = extrapR(d+2, poly[d0+1], rtmp);		// final result after sum-check
	 *v_time_more += (clock() - st);
	 return rvalue;
}
...
void evaluate_circuit(circ* c, uint128*** coeff, long lev){
	clock_t t = clock();
	uint128 val1, val2;
	int di;
	int in1, in2;
...
// to see (before) rounding
	...
	for (int i = c->num_levels-2; i >= 0; --i){
		// cout << i <<" type: "<< c->testtype[i] <<", log sub, num: "<< c->log_sublv_length[i] <<", "<<c->log_sublv_num[i] << endl;
		...
		for(int j = 0; j < c->sublv_length[i]*c->sublv_num[i]; ++j){
			...
			if(type == 2){				// unify
				...
				in1 <<= c->log_sublv_length[i+1];
				c->gates[i][j] = evalPolyVec(coeff[0][j&((1<<di)-1)], c->gates[i+1], in1, (c->sublv_length[i+1]));

				// rounding poly
//				in1 <<= LOG_sqr_ep[LV];
//				c->gates[i][j] = evalPolyVec(coeff[LV][j&((1<<di)-1)], c->gates[i+1], in1, (1<<LOG_sqr_ep[LV]));
			}
	...
	// to see after eval.
		if (c->testtype[i] == 6){
			for(int j = 0; j < print_num; ++j){
				cout << j <<": ";
				print_uint(conv_from_mon(c->gates[i][j], LV));
				cout <<", ";
				if (j == print_num-1)
					cout << endl;
			}
...
circ* construct_circ_init(int total_depth){
	...
	c->rnd_val = (bool*) calloc(total_depth+1, sizeof(bool)); 	// 1 if layer is input/output of rounding. To check if rounding goes well

	return c;
}


//// init_layer: Input layer, level 0: Output layer
void construct_rnd_circ(circ* c, int log_inputs, int init_layer, int init_lev, int* final_layer, int* final_lv, bool initial){
	int n = myPow(2, log_inputs);
	int rnd_depth = 0;
	for (int i = 0; i < GAMMA; ++i)		// for rounding
		rnd_depth += (LOG_ep[init_lev-i]+3);
	*final_layer = init_layer - rnd_depth;
	*final_lv = init_lev - GAMMA;

/// rounding layers (each logp rounding requires logep + 3 depth)
	int depth = init_layer-rnd_depth;
	for (int round = GAMMA; round > 0; --round){
		int sublv_len1 = 1;								// for initial division and eval layer
		int sublv_log1 = 0;
		int sublv_len2 = myPow(2, LOG_sqr_ep[init_lev-round+1]);		// for polygen layer
		int sublv_log2 = LOG_sqr_ep[init_lev-round+1];
		for (int sublv = 0; sublv < LOG_ep[init_lev-round+1]+3; ++sublv){
			int i = sublv + depth;
			...
			else if (sublv < LOG_rem[init_lev-round+1]+3){
				c->sublv_length[i] = sublv_len1;
				c->log_sublv_length[i] = sublv_log1;
				c->sublv_num[i] = n;
				c->log_sublv_num[i] = log_inputs;
				c->gates[i] = (uint128*) calloc(c->sublv_length[i]*c->sublv_num[i], sizeof(uint128));
				if (sublv == LOG_rem[init_lev-round+1]+2){				// eval layer
		...
		depth += LOG_ep[init_lev-round+1]+3;
	}

	if (initial == true){
	/// Input layer (total_depth)
		c->sublv_length[init_layer] = n;
		c->log_sublv_length[init_layer] = log_inputs;
		c->sublv_num[init_layer] = 1;
		c->log_sublv_num[init_layer] = 0;
		c->gates[init_layer] = (uint128*) calloc(n, sizeof(uint128));
	}
}


//// init_layer: Input layer, level 0: Output layer
void construct_poly_circ(circ* c, int log_inputs, int dep_gen, int dep_unif, int init_layer, int init_lev, int* final_layer, int* final_lv, bool initial){
	int n = myPow(2, log_inputs);
	int total_depth = dep_gen+dep_unif+2;

	*final_layer = init_layer - total_depth;
	*final_lv = init_lev;

	int depth = init_layer-total_depth;

	int sublv_len1 = 1;								// for initial division and eval layer
	int sublv_log1 = 0;
	int sublv_len2 = myPow(2, dep_gen);		// for polygen layer
	int sublv_log2 = dep_gen;
	for (int sublv = 0; sublv < total_depth; ++sublv){
		int i = sublv + depth;
		if (sublv < dep_unif+2){
			c->sublv_length[i] = sublv_len1;
			c->log_sublv_length[i] = sublv_log1;
			...
			c->log_sublv_num[i] = log_inputs;
			c->gates[i] = (uint128*) calloc(c->sublv_length[i]*c->sublv_num[i], sizeof(uint128));
			if (sublv == dep_unif+1){				// eval layer
				c->in1[i] = in1evalalpha;
				c->in2[i] = in2evalalpha;
				c->types[i] = eval5alpha;
			}
			else if (sublv == 0){							// extract layer
				c->in1[i] = in1eval;
				c->in2[i] = in2eval;
				c->types[i] = extract6;
			}
...
void generate_input(circ* c, int num_input, int layer_input, int bit_input){
	...
	for(int i = 0; i < total_gates; i++){
		c->gates[layer_input][i] = conv_to_mon(rand_128(unif, &gen)%PRIMES[bit_input*GAMMA], LV);			// generate random 2gamma p-digit (~ 2eta bit) integers
	}
}


void verif_rnd_layer(circ* c, int d, int init_layer, int* final_layer, rElt** gateVals, rElt** poly, rElt* betavals, rElt* ri, rElt* z, rElt* r, rElt* temp, uint128** LDpoly, int* com_ct, int* rd_ct, clock_t* v_pretime, clock_t* v_time_more){

	cout << "rounding ver start! "<< endl;

	*(final_layer) = init_layer; // init_layer;
	for (int i = 0; i < GAMMA; ++i)		// for rounding
		*(final_layer) += (LOG_ep[LV+i+1]+3);


	for (int i = init_layer; i < *(final_layer); ++i){
	//	cout << i << " "<<c->testtype[i] <<" " << LV << endl;
		if (c->testtype[i] == 4){										// initial division layer
			for (int i = 0; i < DIM; ++i){
				ri->c[i]=ri->c[i]*PRIME;
			}
			LV++;
		...
		else if (c->testtype[i] == 5){			// alpha eval layer
			*(ri) = sum_check_rouding_alphaR(gateVals[i+1], *(ri), c->log_sublv_num[i], c->log_sublv_length[i], c->log_sublv_length[i+1], com_ct, rd_ct, v_pretime, v_time_more, LDpoly[LV], r, poly, betavals, z, LV);
			temp = z;
			z=r;
			r=temp;
		}
		else if (c->testtype[i] == 6){						// extract layer
			for(int j = d; j > 0; --j)
				z[j] = z[j-1];
			z[0] = ZERO;
		}
		else if (c->testtype[i] == 2){										// unify layers
			*(ri)=sum_check_rouding_unifyR(gateVals[i+1], *(ri), c->log_sublv_num[i], c->log_sublv_length[i], v_time_more, com_ct, rd_ct, r, poly, betavals, z, LV);
			temp = z;
			z=r;
			r=temp;
		}
		else if (c->testtype[i] == 3){	// polygen layers
...
void verif_poly_layer(circ* c, int d, int init_layer, int* final_layer, int dep_gen, int dep_unif, rElt** gateVals, rElt** poly, rElt* betavals, rElt* ri, rElt* z, rElt* r, rElt* temp, uint128** LDpoly, int* com_ct, int* rd_ct, clock_t* v_pretime, clock_t* v_time_more){
...
//	for (int i = 0; i < GAMMA; ++i)		// for rounding
//		*(final_layer) += (LOG_ep[LV+i+1]+3);
	*(final_layer) += (dep_gen + dep_unif + 2);
	...
	for (int i = init_layer; i < *(final_layer); ++i){
		...
		if (c->testtype[i] == 4){										// initial division layer
			...
			*(ri) = sum_check_rouding_alphaR(gateVals[i+1], *(ri), c->log_sublv_num[i], c->log_sublv_length[i], c->log_sublv_length[i+1], com_ct, rd_ct, v_pretime, v_time_more, LDpoly[0], r, poly, betavals, z, LV);
			temp = z;
			z=r;
			r=temp;
		}
...
////Test rounding and LDpoly
	...
	cout << "simrounding: "; print_uint(conv_from_mon(out2, E)); cout << endl;
	...
	cout << endl;
	cout << "rounding of 2^" << d <<" ("<< pow(2,d)<<") elements start!"<<endl;
...
/********************************************/
	...
	circ* c;
	int dep_gen = atoi(argv[2]);
	int dep_unif = atoi(argv[3]);
	int total_depth = 0;
//	for (int i = 0; i < 3*GAMMA; ++i)		// total_depth calculation (for rounding)
//		total_depth += (LOG_ep[E-i]+3);
	total_depth += (dep_gen + dep_unif + 2);

	int layer_st = total_depth;
	int lv_st = E;
	int layer_end = 0;
	int lv_end = 0;
...
//// construct circuit
	...
	construct_poly_circ(c, d, dep_gen, dep_unif, layer_st, lv_st, &layer_end, &lv_end, true);

//	construct_rnd_circ(c, d, layer_st, lv_st, &layer_end, &lv_end, true);
//	layer_st = layer_end;
//	lv_st = lv_end;
//	construct_rnd_circ(c, d, layer_st, lv_st, &layer_end, &lv_end, false);
//	layer_st = layer_end;
//	lv_st = lv_end;
//	construct_rnd_circ(c, d, layer_st, lv_st, &layer_end, &lv_end, false);
...
//// generate input (at layer : total_depth)
	t = clock();
	generate_input(c, myPow(2, d), total_depth, 1);
	cout << "time to randomly generate data is: " << (double) ((double)clock()-(double)t)/(double) CLOCKS_PER_SEC << endl;


// Test gates label
	for (long i = c->num_levels-1; i >= 0; --i)
		cout << i <<": "<< c->log_sublv_length[i] <<" type: " << c->testtype[i] << endl; //c->log_level_length[i]) << endl;
...
/************************************************************/
	...
	uint128** Epoly = new uint128*[0]();
	Epoly[0] = new uint128[1<<(dep_unif+dep_gen+1)]();
	uint128*** Evalpoly = new uint128**[1]();
	Evalpoly[0] = new uint128*[1<<(dep_unif+1)]();
	int ll = 0;
	for (long j = 0; j < (1<<(dep_unif+1)); ++j){
		Evalpoly[0][j] = new uint128[1<<dep_gen]();
		for (long k = 0; k < (1<<dep_gen); ++k){
			if (j < (1<<dep_unif))
				Evalpoly[0][j][k] = conv_to_mon(rand_128(unif, &gen)%PRIMES[LV], LV); // CST[1][LV];
			else
				Evalpoly[0][j][k] = CST[0][LV];
			Epoly[0][ll] = Evalpoly[0][j][k];
			ll++;
		}
	}
	Evalpoly[0][(1<<(dep_unif+1))-1][(1<<dep_gen)-1] = CST[1][LV];
	Epoly[0][ll-1] = Evalpoly[0][(1<<(dep_unif+1))-1][(1<<dep_gen)-1];

//evaluate the circuit
	evaluate_circuit(c, Evalpoly, E);



	long max_layer_size = n*EP2;
	long max_layer_log = d+LOG_ep[E]+1;
...
/// Verifier evaluates a random point in the MLE of the output   (0 layer)
	t=clock();
	rElt ri = evaluate_V_iR(d, n, gateVals[0], z, LV);
...
/// Start GKR protocol with Prover
	clock_t ct=0;
	clock_t pt=0;
	int com_ct=n; //counting the answer as communication here
	int rd_ct=1;
	rElt ri0=ZERO;
	rElt ri1=ZERO;
	rElt* temp = new rElt[1]();


//	cout << "poly eval ver start! "<< endl;
	layer_st = 0;
	verif_poly_layer(c, d, layer_st, &layer_end, dep_gen, dep_unif, gateVals, poly, betavals, &ri, z, r, temp, Epoly, &com_ct, &rd_ct, &v_pretime, &v_time_more);

//	cout <<"l end:" <<layer_end <<endl;

//// Verification of rounding layer
//	cout << "rounding ver start! "<< endl;
//	layer_st = 0;
//	verif_rnd_layer(c, d, layer_st, &layer_end, gateVals, poly, betavals, &ri, z, r, temp, LDpoly, &com_ct, &rd_ct, &v_pretime);
//	layer_st = layer_end;
//	verif_rnd_layer(c, d, layer_st, &layer_end, gateVals, poly, betavals, &ri, z, r, temp, LDpoly, &com_ct, &rd_ct, &v_pretime);
//	layer_st = layer_end;
//	verif_rnd_layer(c, d, layer_st, &layer_end, gateVals, poly, betavals, &ri, z, r, temp, LDpoly, &com_ct, &rd_ct, &v_pretime);
	...
	t = clock();
	ri1 = evaluate_V_iR(d, n, Vcopy, z, LV);
	if(!eqTest(ri, ri1)){
		cout << "in very last check, ri != ri1. ri was : ";print_uintR(ri);
		cout << " and ri1 was ";print_uintR(ri1);
		cout << endl;
	}

	cout << "Vf online time is: " << (double)(v_time + (double) clock()-t)/CLOCKS_PER_SEC <<" sec"<< endl;
	cout << "Vf abbrev time is: " << (double) v_time_more/CLOCKS_PER_SEC <<" sec" << endl;
