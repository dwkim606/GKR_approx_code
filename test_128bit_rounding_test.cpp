...
////Test rounding and LDpoly
	...
	cout <<"eval. time: " << end-st << endl;
	st = clock();
	uint128 out2 = simRounding(conv_to_mon(input, E), E);
	end = clock();
	cout <<"rnd. time: " << end-st << endl;
...
/********************************************/
	...
	for (int i = 0; i < GAMMA; ++i)		// total_depth calculation (for rounding)
		total_depth += (LOG_ep[E-i]+3);
	int layer_pos = total_depth;

//// construct circuit
	clock_t t = clock();
	c = construct_circ_init(total_depth);
	c->round_level_st = total_depth-1;
	construct_rnd_circ(c, layer_pos, d, true);
	cout << "\n circuit construct time is: " << (double)((double) clock()-t)/CLOCKS_PER_SEC << endl;

//// generate input (at layer : total_depth)
	t = clock();
	generate_input(c, myPow(2, d), total_depth, 2);
	cout << "time to randomly generate data is: " << (double) ((double)clock()-(double)t)/(double) CLOCKS_PER_SEC << endl;


	uint128* test = new uint128[n]();
	for (int i = 0; i < n; ++i){
		test[i] = conv_from_mon(c->gates[total_depth][i], E);
	}
	uint128 tmp = myPow(PRIME, GAMMA);
	uint128 tmp2 = (tmp-1)/2;
	uint128 tmp3 = 0;
	uint128 rndtmp = 1 << (ETA-1);
	t = clock();
	for (int i = 0; i < n; ++i){
		test[i] = (test[i]  + rndtmp) >> ETA;
		// tmp3 = test[i]%tmp;
		// if (tmp3 > tmp2)
		// 	test[i] += (tmp - tmp3);
		// else
	...
	cout << "time to naive rounding is: " << (double) ((double)clock()-(double)t)/(double) CLOCKS_PER_SEC << endl;

////Test gates label
//	for (long i = c->num_levels-1; i >= 0; --i)
//		cout << i <<": "<< c->log_level_length[i] <<" type: " << c->testtype[i] << endl; //c->log_level_length[i]) << endl;
...
//evaluate the circuit
	evaluate_circuit(c, partLDpoly, E);
